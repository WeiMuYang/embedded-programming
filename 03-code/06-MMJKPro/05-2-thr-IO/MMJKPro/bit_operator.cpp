...
BitOperator::BitOperator(QObject *parent) : QObject(parent)
{
 ...
 */
void BitOperator::separatePrintBit(unsigned char byteData){
    int arr[8] = {0};
    int bit = 0;
    // 进行的位的与运算，一个是1则是1
    for(int i = 0; i < 8; ++i){
 ...
 */
void BitOperator::changeBytePosBit(unsigned char *pByteData, int pos, int bit){
    // 位移动运算
 ...
 * @param pbit:位的值    [0,1]
 */
void BitOperator::getBytePosBit(unsigned char byteData, int pos, int *pbit){
    switch (pos) {
    case 0:
